...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        grad_joint = sum_node_list(node_to_output_grads_list[node])
        node.grad = grad_joint
        if node.op is None:
            continue
        partial_grads = node.op.gradient_as_tuple(grad_joint, node)
        for inode, p_grad in zip(node.inputs, partial_grads):
            if inode not in node_to_output_grads_list:
                node_to_output_grads_list[inode] = []
            node_to_output_grads_list[inode].append(p_grad)
    ### END YOUR SOLUTION
